...
type Step = 'pick-week' | 'enforced' | 'groups';

/**
 * Copie, sous réserve de faisabilité, la préparation (cours enforced + groupes) d'une semaine
 * source vers la semaine actuellement ouverte (destination). Un cours de S n'est copiable que
 * s'il existe en D un cours "similaire" (`courseSimilarityKey` — même code/type/durée/
 * enseignant(s)/groupe(s), semaine exclue) ; un groupe n'est copiable que si TOUS ses membres
 * le sont. Les enforced sont appliqués d'abord, les groupes ensuite (un groupe enforced a
 * chacune de ses tâches enforced — le flux les traite donc de façon transparente).
 */
export function CopyWeekPrepModal() {
  ...
  function handleApply() {
    ...
    // enforced individuellement — le flux les traite donc de façon transparente).
        ...
        <DialogFooter>
          ...
          {step === 'enforced' && (
            <>
              ...
              <Button type="button" onClick={() => setStep('groups')}>Suivant</Button>
            </>
          )}
          {step === 'groups' && (
            <>
