...
export function SchoolYearBlock() {
  ...
  const setSchoolYearConfig = useSchedulerStore((s) => s.setSchoolYearConfig);

  const availableYears = getAvailableSchoolYears();
  const [selectedYear, setSelectedYear] = useState<string>(
    schoolYearConfig?.year ?? availableYears[1] ?? availableYears[0] ?? '',
  );
  const [selectedZone, setSelectedZone] = useState<'A' | 'B' | 'C'>(
    schoolYearConfig?.zone ?? 'A',
  );
  ...
  function handleClear() {
    setSchoolYearConfig(null);
    setStatus('idle');
  }

  const isLoaded =
    schoolYearConfig !== null &&
    schoolYearConfig.year === selectedYear &&
    schoolYearConfig.zone === selectedZone;

  const vacationCount = schoolYearConfig?.periods.filter((p) => p.type === 'vacation').length ?? 0;
  const holidayCount = schoolYearConfig?.periods.filter((p) => p.type === 'public-holiday').length ?? 0;
  ...
  return (
    <div className="flex flex-col gap-4 rounded-lg border border-border bg-card p-6">
      ...
      {isLoaded && status !== 'error' && (
        <div className="flex items-center gap-2 rounded-md border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-950/30 px-3 py-2">
          ...
          <span className="text-xs text-green-700 dark:text-green-300">
            <span className="font-medium">{schoolYearConfig!.year} — Zone {schoolYearConfig!.zone}</span>
            {' · '}
      ...
      {!isLoaded && schoolYearConfig && status !== 'loading' && status !== 'error' && (
        <div className="flex items-center gap-2 rounded-md border border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-950/30 px-3 py-2">
          ...
          <span className="text-xs text-yellow-700 dark:text-yellow-300">
            Données chargées pour {schoolYearConfig.year} — Zone {schoolYearConfig.zone}. Cliquez &laquo; Charger &raquo; pour mettre à jour.
          </span>
        </div>
      )}
