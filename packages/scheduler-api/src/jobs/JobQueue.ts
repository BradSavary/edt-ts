...
// Bundle le worker TypeScript en CJS au démarrage (tsx ne propage pas ses hooks
// de résolution .js→.ts dans les worker threads — esbuild contourne ce problème).
...
const { outputFiles } = buildSync({
  ...
  platform: 'node',
  write: false,
  external: ['node:*'],   // ne pas bundler les built-ins Node
  logLevel: 'warning',
});
const WORKER_CODE = outputFiles[0].text;
console.log('[JobQueue] Worker bundlé avec esbuild (%d bytes)', WORKER_CODE.length);
