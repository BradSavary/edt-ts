...
> **STATUT FINAL (2026-07-16) : LIVRÉ — le verdict STOP initial a été renversé après contre-enquête sur données fraîches.**
>
> *Historique en deux temps.* (1) L'exécution par Sonnet était fidèle au plan (5 tests verts, dont une preuve white-box du réordonnancement ; 37/38/39 byte-identiques off/on), mais la validation semaine 40 sur l'instantané de données d'alors montrait une régression (5 neutralisées/42,9s contre 2/14,5s) → STOP conforme au §5, rien commité. (2) Frédéric a ensuite testé COS dans l'app sur son projet **vivant** et obtenait l'inverse. La contre-enquête (Fable) a montré que la « régression » était un artefact d'**instantané périmé** : sur l'export frais du projet, COS gagne partout sur la semaine 40 — 3/2/2 neutralisées aux budgets 1000/3000/10000 itérations (runs déterministes) contre 5/4/4 en baseline, et **2 neutralisées/15,8s contre 4/34,6s en config production** (10s/round) — tout en restant strictement identique sur 37/38/39 (pipeline weekSaves complet rejoué). Décision de Frédéric : commit du mécanisme derrière son flag + exposition dans le dialogue de configuration.
>
> *Enseignements consignés (détail dans la mémoire de session `project_backtracking_thrashing_backjumping.md`) :* l'effet de COS transite par l'échantillonnage de `_failureCounts` sous troncature des rounds — il peut aider ou nuire selon l'état de l'instance (les deux directions ont été observées, chacune parfaitement reproductible sur son jeu de données) ; dans les rounds infaisables ~50% des unités finissent horodatées et le débit par nœud est divisé par ~3, sans que cela prédise le signe de l'effet net ; un instantané de données réelles se périme — ré-exporter le projet avant toute campagne de validation ; borner les rounds en itérations plutôt qu'en secondes rendrait `solveWithElimination` déterministe inter-machines (piste future). Note de calibration des tests : construire un scénario synthétique où COS gagne nettement s'est avéré structurellement difficile (COS ne reprioritise que ce qui échoue *personnellement*) ; le test central vérifie donc le mécanisme directement (white-box) plutôt qu'un gain combinatoire.
