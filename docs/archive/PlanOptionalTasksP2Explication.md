...
> **STATUT (2026-07-18, exécution par Sonnet) : LIVRÉ, prêt à committer.**
>
> **Brique 1** : `_explainInheritedResult` implémentée conforme au plan (rejeu déterministe de la pile de placement, garde de fidélité, repli sûr). Précision non explicite dans le plan mais nécessaire : `initSolver()` seul ne peuple ni `_solution` ni `_scheduled` ni l'usage quotidien — il faut AUSSI `_resetBacktrackState()` (même paire d'appels que celle déjà faite avant la passe B&B). `_iterations` sauvegardé/restauré autour de cet appel pour ne pas écraser le compteur de la passe B&B déjà loggé. 7 tests (4 nouveaux + 3 adaptés), suite complète intacte (100/100 scheduler-core, dont le test "cascade de dépendants" dont les raisons attendues basculent de "Unité la plus bloquante"/"Dépend de" vers MUS/"Sautée par cascade").
>
> **Brique 2** : module `resourceLoadAnalysis.ts` (2 modes, capacité via `AvailabilityManager.getAvailability()` — même primitif que `analyzeConstraints`, pas de troisième source de vérité) + composant `ResourceLoadPopover` monté dans `CourseConstraintList` (préparation, toutes les cartes) et `SidebarAnalysis` (sautées non pré-neutralisées). 9 tests unitaires dont les deux cas LAVEFVE miniature (préparation : ratio de tension ; analyse : volume suffisant mais `anyDayFits=false`). 266/266 scheduler-client.
>
> **Validation réelle S40** : les raisons MUS sont bien produites sur le chemin hérité — ex. `R3.04_MEUNIER Sandrine_BUT2-G1_BUT2-G2` (COS on) : « créneaux nécessaires occupés par R3.18_ROUX Maxence_BUT2-G1_BUT2-G2 » — lisible et actionnable. UI vérifiée en Playwright : 110 popovers en préparation (table demande/capacité, dont une découverte réelle — la salle "R04" à 205% de demande hebdo, un signal que l'ancien classement 🔴/🟠/🟢 teacher-only ne montrait pas) et le popover en analyse (charge/mou), zéro erreur console sur tout le parcours import→préparation→planification→analyse.
>
> **Correction post-livraison (2026-07-19, trouvée par Frédéric)** : le premier compte-rendu de validation S40 affirmait à tort `anyDayFits=true` pour R3.04 LAVEFVE (jeudi, 4h de mou) — Frédéric a signalé que les salles ne sont disponibles que 8h-12h30 le jeudi, contredisant ce calcul. Root cause : le script de validation testait seulement les 2 groupes candidats (BUT2-G1/G2), pas l'enseignant ni les 3 salles — un test incomplet, pas un bug de calcul de capacité (`capacityByDay` reflète correctement les 4h30 du jeudi une fois vérifié sur toutes les ressources). Reprise avec l'ensemble RÉEL des candidats (`serializeNeutralizedUnit`, 6 ressources) : chaque ressource individuelle a bien SON jour de mou (enseignant : lun/mar/mer/ven mais pas jeudi 0h30<1h30 requis ; les 2 groupes : SEULEMENT jeudi) — mais AUCUN jour n'est commun aux 6. Ce contrôle a exposé un vrai bug du composant livré : le bandeau de synthèse testait `rows.every(r => !r.anyDayFits)` (toutes les ressources en échec sur TOUS les jours), une condition trop faible qui reste `false` — donc AUCUN bandeau — dès qu'une ressource a un jour qui lui convient individuellement, même si ce jour diffère d'une ressource à l'autre. **Corrigé** : nouvelle fonction `hasCommonFeasibleDay(rows)` (jour commun à TOUTES les ressources simultanément), c'est la seule question qui compte pour la faisabilité réelle d'un placement — le bandeau utilise désormais cette vérification croisée. 3 tests nouveaux dont la reproduction exacte du cas S40 (chaque ressource a son jour, mais jamais le même → `false`). Le diagnostic corrigé confirme finalement l'intuition manuelle de Frédéric (granularité/fragmentation insuffisante, cette fois à travers plusieurs dimensions de ressources combinées) — l'écart initial venait entièrement du test de validation, pas du diagnostic métier de Frédéric ni, après correction, du composant.
>
> Reste avant commit : rien — commit unique à suivre.
>
> **STATUT RÉVISION (2026-07-19, section R exécutée par Sonnet) : LIVRÉ, prêt à committer.** `_explainInheritedResult` (rejeu déterministe) et `_explainSkip` (MUS) supprimés en entier. Chemin hérité : nouvelle méthode pure `_genericizeReasons` (pas de reconstruction d'état, pas de rejeu) — raison générique uniforme `GENERIC_UNPLACEABLE_REASON`, cascade exceptée (`cascadeReason`, factorisée et réutilisée par `_recordIncumbent` aussi). Docstrings de classe, `solveWithElimination`, blâme informatif mis à jour. Tests : bloc « MUS pour le résultat hérité » (3 tests) supprimé, « cascade de dépendants » adapté (texte générique exact au lieu de « structurellement insuffisantes »), 1 nouveau test d'uniformité (« chemin hérité : toutes les raisons non-cascade sont le texte générique exact » — vérifie aussi l'absence de texte gourmand/MUS). 17/17 core (98/98 suite complète), 269/269 client (brique 2 intouchée), typecheck clean. `docs/PlanComboBranchementBB.md` mis à jour (précondition rejeu devenue sans objet). Sanity S40 réelle : raisons 100% génériques/cascade confirmées (`R3.04_DUBOIS`, `R3.04_LAVEFVE` → texte générique exact, plus aucun MUS).
