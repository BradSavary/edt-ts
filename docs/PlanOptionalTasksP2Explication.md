...
>
> **Validation réelle S40** : les raisons MUS sont bien produites sur le chemin hérité — ex. `R3.04_MEUNIER Sandrine_BUT2-G1_BUT2-G2` (COS on) : « créneaux nécessaires occupés par R3.18_ROUX Maxence_BUT2-G1_BUT2-G2 » — lisible et actionnable. **Écart honnête avec le critère §3.2** : la table de charge de ce cours montre 32h/37,5h (85%, même ordre de grandeur que l'estimation manuelle de Frédéric ≈33,5h/34,5h) mais `anyDayFits=true` (jeudi a 4h de mou) — ce qui CONTREDIT le "aucun jour avec mou ≥1,5h" du diagnostic manuel. Root cause identifiée : dans cette config précise, le vrai blocage de R3.04 est un conflit d'OCCUPATION avec R3.18 VEILLON (même groupes, même créneau disputé), pas un mur de capacité — un diagnostic différent mais plus précis et plus actionnable que l'hypothèse de granularité. La table de charge fonctionne correctement (mécanisme vérifié indépendamment par les 2 cas LAVEFVE-miniature unitaires, qui reproduisent exactement le phénomène "volume OK, granularité insuffisante" quand IL a lieu) ; elle donne simplement un diagnostic différent sur CE cours précis parce que la cause réelle est différente de celle supposée. UI vérifiée en Playwright : 110 popovers en préparation (table demande/capacité, dont une découverte réelle — la salle "R04" à 205% de demande hebdo, un signal que l'ancien classement 🔴/🟠/🟢 teacher-only ne montrait pas) et le popover en analyse (charge/mou), zéro erreur console sur tout le parcours import→préparation→planification→analyse.
>
