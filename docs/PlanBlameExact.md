...
> **STATUT (2026-07-17) : exécuté par Sonnet, analysé par Fable.** Implémentation fidèle (7 tests verts, 81/81 + 257/257, typecheck clean) + un correctif nécessaire non prévu : symétrie du round-trip `unBook`/`book` de `TaskGroupUnit` (restauration de `_pendingAssignment`, bug latent prouvé avant correction). Validation réelle : **seul (COS off), le blâme exact n'est jamais pire et parfois meilleur** (semaine 40 : 5→4 neutralisées au budget 1000 ; 37/38/39 byte-identiques) — critères du §5 remplis sur cette colonne. **Combiné à `conflictOrderingSearch`, il régresse** (3 neutralisées vs 2 aux budgets 3000/10000) → STOP appliqué par Sonnet conformément au §5.
>
> *Cause racine, établie par instrumentation (rounds déterministes, round 0 strictement identique off/on — 509 itérations — prouvant au passage que la restauration d'état du contrefactuel est exacte)* : ce n'est **pas un bug**. Le blâme exact désigne comme cible round 1 l'unité R1.04 DUBOIS #9, **contrefactuellement prouvée coupable minimale 280 fois** — le signal est correct. Mais « coupable minimal le plus fréquent dans le préfixe de recherche » n'est pas le bon objectif pour choisir la tâche à retirer : l'éliminer cascade à 3 neutralisées, alors que la sur-inclusion diffuse de l'ancien scan (424 sur R3.04 #67 en agrégeant toute la « pression DUBOIS ») tombe par chance sur une meilleure cible (2 neutralisées). C'est la **4e interaction non-monotone** observée, toutes passant par le choix gourmand irréversible de `solveWithElimination` — la correction durable est le chantier « tâches optionnelles / branch-and-bound sur les sauts », pas un raffinement du blâme.
>
> *Limites documentées* : (1) la promesse du §1 « corrige gratuitement l'angle mort maxDailyMinutes » est fausse pour les impasses atteintes via la boucle de rejets de filtres (fromTime au bord de fenêtre → sonde aveugle) — découvert par Sonnet, documenté dans le test 5 ; l'artefact est réel mais marginal (24 self-blames sur 304 incréments dans le round mesuré). Une v2 (« fenêtre de blâme » = depuis le dernier placement réussi plutôt que le fromTime avancé par filtres) corrigerait quota et bord-de-fenêtre, mais ne changerait pas le verdict de l'interaction COS (280 ≫ 24) — différée. (2) Ne pas combiner `conflictSetExact` et `conflictOrderingSearch` en l'état ; la config pratique du jour reste COS on + exact off.
>
> *Décision : commité sur validation de Frédéric — valeur = outillage vérifié pour le chantier tâches optionnelles (un B&B dirigé par les conflits exige des conflits minimaux exacts), fix TaskGroupUnit, tests permanents ; flag inerte par défaut.*
